...
class PowerDetailPage extends StatefulWidget {
  PowerDetailPage({Key key, this.deviceModel}) : super(key: key);
  final DeviceModel deviceModel;

  @override
  _PowerDetailPageState createState() => _PowerDetailPageState();
}
...
class _PowerDetailPageState extends State<PowerDetailPage> {
  ...
  void _onRefresh() async {
    print('@@@ PowerDetailPage._onRefresh() _isRefreshable : $_isRefreshable');
  ...
  void _onLoading() async {
    print(
        '@@@ PowerDetailPage._onLoading() _pagecount : $_pagecount , _pn : $_pn');
  ...
  void initState() {
    ...
    _controller.addListener(() {
      // print(
      //     "@@@ => PowerDetailPage._controller.Listener() _controller.position.pixels : ${_controller.position.pixels} ");
    ...
    //   if (event.page == 'PowerDetailPage') {
    //     _getData();
    //   }
    // });
    // _getData();
    print("@@@ => PowerDetailPage.initState() ... ");
  }
...
// class PowerDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print("@@@ PowerDetailPage.build() ...");

    return Scaffold(
      appBar: AppBar(
        // backgroundColor: AppStyle.mainBackgroundColor,
        title: Text('耗电明细 [${widget.deviceModel.name}]'), //耗电量报表
        elevation: 0.0,
...
//耗电量
  ...
  _getFABHeight() {
    // print("@@@ PowerDetailPage._getFABHeight() _dFABHeight : $_dFABHeight");
...
// return _controller.position.pixels>400?40.0:0.0;
  ...
  _getBodyUI() {
    // print('@@@ PowerDetailPage._getBodyUI() begin ... ');
    if (_lstData == null)
...
//按钮
  ...
  _getData() async {
    ...
    print('@@@ PowerDetailPage._getData _pagecount : $_pagecount , _pn : $_pn');

    //  GlobalVar.userInfo.id + 'templates';
    // if (_pn == 0) {}
    String cpids = '';
    widget.deviceModel.lstCP.forEach((cpm) {
      cpids = cpids + '-${cpm.uuid}';
    });
    if (_pagecount < _pn) return;
//"ids",  "qt", "dt", "begindt","enddt", "pageno", "orderby"
    ...
    await DataHelpr.dataQuerier('Power/ControlPanel', param, [PowerListModel()],
        (ret) {
      HttpRetModel rm = ret as HttpRetModel;
      print(
          '@@@ PowerDetailPage._getData cloud data rm.pagecount : ${rm.pagecount} , rm.totalcount : ${rm.totalcount} , data.length : ${rm.data.length}');
      if (rm.ret == 0) {
      ...
      // print(
      //     '@@@ PowerDetailPage._getData _lstData.length : ${_lstData.length}, _pn:$_pn');
    });
